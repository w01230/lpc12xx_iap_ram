...
static int iap_prepare_for_write(unsigned long start, unsigned long end)
{
...
static int iap_erase_sector(unsigned long start, unsigned long end, unsigned long cpu_khz)
{
	struct iap_struct iap_data;
...
static int iap_blankcheck_sector(unsigned long start, unsigned long end)
{
	struct iap_struct iap_data;
...
//static int iap_erase_info_page(unsigned long start, unsigned long end, unsigned long cpu_khz)
//{
//	struct iap_struct iap_data;
//	
//	iap_data.command = ERASE_INFO_PAGE;
//	iap_data.parameter[0] = start;
//	iap_data.parameter[1] = end;
//	iap_data.parameter[2] = cpu_khz;
//	iap_operation(&iap_data.command, &iap_data.state);
...
static int flash_blankcheck(struct boot_data_struct *boot_data)
{
	...
	state = iap_blankcheck_sector(start, end);
...
static int flash_erase(struct boot_data_struct *boot_data)
{
	...
	state = iap_prepare_for_write(start, end);
	state |= iap_erase_sector(start, end, cpu_khz);
...
const struct boot_handler flash_handler = {
